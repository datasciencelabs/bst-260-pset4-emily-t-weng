...
response #200 OK means successful HTTP request
```
...
population <- population |>
  row_to_names(row_number = 1) |>  #row to names
  as_tibble() |>  #convert to tibble
  select(-state) |>  #remove state column
  rename(state_name = NAME) |>  #rename NAME to state_name
  pivot_longer(
    cols = starts_with("POP_"),
    names_to = "year",
    values_to = "population"
  ) |>  #tidy the data with pivot_longer
  mutate(
    year = str_remove(year, "POP_"),  #remove POP_ prefix
    year = parse_number(year),  #parse year as numeric
    population = parse_number(population)  #parse population as numeric
  ) |>
  mutate(
    state = state.abb[match(state_name, state.name)]  #add state abbreviations
  ) |>
  mutate(
    state = case_when(
      state_name == "District of Columbia" ~ "DC",
      state_name == "Puerto Rico" ~ "PR",
      TRUE ~ state  #add DC, PR
    )
  )
```
...
```{r}
population |> 
  mutate(state_name = reorder(state_name, population)) |>  #reorder state by population
...
No, not all the data is there, the API of the CDC likely limits the amount of records to 1000 to prevent overload.
...
library(knitr)
cases |>
  mutate(date = ymd(date)) |>  # Parse date using lubridate
  mutate(
    year = year(date),
